...
import it.pagopa.pdnd.interop.uservice.partymanagement.api.HealthApi
import it.pagopa.pdnd.interop.uservice.partymanagement.api.PartyApi
...
class Controller(
  ...
  def getBuildInfo: Route =
    path("pdnd-interop-uservice-party-management" / "build-info") {
      get {
        complete(HttpEntity(ContentTypes.`application/json`, buildinfo.BuildInfo.toJson))
      }
    }

  lazy val routes: Route = getBuildInfo ~ pathPrefix("pdnd-interop-uservice-party-management" / interfaceVersion) {
    toStrictEntity(strictnessTimeout) {
      extractRequest { request =>
        validationFunction(request) {
          health.route ~ party.route
        }
