...
import akka.projection.{HandlerRecoveryStrategy, ProjectionId}
...
class ProjectionContractsHandler(
  ...
  private def checkRelationshipConfirmed(event: PartyRelationshipWithId, queueEvent: QueueEvent): DBIO[Done] = {
    ...
    val result = for {
      ...
      _     <- found match {
        case Some(partyRelationship) if partyRelationship.role == PartyRole.MANAGER =>
    ...
    result.onComplete {
      case Failure(e) =>
        logger.info( // apz debug
          s"Error projecting confirmation of relationship having id ${event.partyRelationshipId} on queue",
          e
        )
        DBIOAction.failed(e)
      case Success(_) =>
