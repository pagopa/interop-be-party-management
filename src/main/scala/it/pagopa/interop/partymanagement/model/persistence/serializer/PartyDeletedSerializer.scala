...
import it.pagopa.interop.partymanagement.model.persistence.PartyDeleted
...
class PartyDeletedSerializer extends SerializerWithStringManifest {
  ...
  final val className: String = classOf[PartyDeleted].getName

  override def toBinary(o: AnyRef): Array[Byte] = o match {
    case event: PartyDeleted => serialize(event, className, currentVersion)
    case _ =>
