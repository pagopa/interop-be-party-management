...
class PartyApiServiceSpec extends ScalaTestWithActorTestKit(PartyApiServiceSpec.config) with AnyWordSpecLike {
  ...
  "Working on relationships" must {
    ...
    "create a new relationship" in {
      ...
      val orgSeed =
        OrganizationSeed(institutionId, None, "Institutions One", "mail1@mail.org", "fiscalCode", Set.empty, Seq.empty)
      val rlSeed =
        RelationshipSeed(
          from = personUuid,
          to = orgUuid,
  ...
  "Suspending relationship" must {
    ...
    "succeed" in {
      ...
      updatedRelationship.status shouldBe RelationshipStatus.SUSPENDED
  ...
  "Activating relationship" must {
    ...
    "succeed" in {
      ...
      relationshipResponse.status shouldBe StatusCodes.OK
      val updatedRelationship = Unmarshal(relationshipResponse.entity).to[Relationship].futureValue
      updatedRelationship.status shouldBe RelationshipStatus.ACTIVE
  ...
  "Deleting relationship" must {
    ...
    "succeed" in {
      ...
      relationshipResponse.status shouldBe StatusCodes.OK
      val updatedRelationship = Unmarshal(relationshipResponse.entity).to[Relationship].futureValue
      updatedRelationship.status shouldBe RelationshipStatus.DELETED
  ...
  "Working on token" must {
    ...
    "invalidate a token" in {
      ...
      body.items.map(_.status) should contain only RelationshipStatus.REJECTED
